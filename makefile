...
tf-workspace-staging:
	docker-compose -f terraform/docker-compose.yml run --rm terraform workspace select staging

.PHONY: tf-workspace-prod
tf-workspace-prod:
	docker-compose -f terraform/docker-compose.yml run --rm terraform workspace select prod
